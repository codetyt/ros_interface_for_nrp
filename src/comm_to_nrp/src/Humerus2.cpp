...
public:
  SubscribeAndPublish()
  {
    pub_ = n_.advertise<std_msgs::Float64>("/gazebo_muscle_interface/robot/Humerus2/cmd_activation", 1000);
  ...
  void callback(const std_msgs::Float64MultiArray input)
  {
    ...
    pub_.publish(output);
    printf("for_Humerus2 = %lf, published\n", output.data);
  }
...
{
  ros::init(argc, argv, "for_Humerus2");
